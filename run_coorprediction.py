...
#输出文件路径
flags.DEFINE_string(
    "output_dir", "/home/zju/xlc/bert/bert-master/regressResult/finalResult/100%Semantic3Laters768-768-768-noFreeze",
...
#初始化模型保存
flags.DEFINE_string(
    # "init_checkpoint", "./Address20190113/bert-model/bert_model.ckpt",
    ...
    # "init_checkpoint", "/home/zju/mrc/machine-learning/Address20190113/output0124/model.ckpt-30000",
...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, results=None):
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, the_results):
...
class ColaProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "final_real_right_regress_eval.tsv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "final_real_right_regress_predict.tsv")), "test")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      #   results = [0.0,0.0]
      # else:
      #   results = [float(line[1])-1090000, float(line[2])-3360000]
      results = [((float(line[1]) - 1090000)), ((float(line[2]) - 3360000))]
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=None, results=results))
    return examples
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["the_results"] = create_float_feature(feature.the_results)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    # fourthLayer = tf.layers.dense(
    #     thirdLayer,
    #     768,
    #     activation=tf.nn.relu,
    #     kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
    # fourthLayer = tf.nn.dropout(fourthLayer, keep_prob=0.9)
    # fifthLayer = tf.layers.dense(
    #     fourthLayer,
    #     768,
    #     activation=tf.nn.relu,
    #     kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
    # fifthLayer = tf.nn.dropout(fifthLayer, keep_prob=0.9)
    outputResult = tf.layers.dense(
        thirdLayer,
            2)
#     hidden_size = output_layer.shape[-1].value
#     inputLayer = tf.layers.dense(
#             output_layer,
#             50000,
#             activation=tf.tanh,
#             kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
...
#         2)
    ...
    #     "output_weights", [2, 5000],
    #     initializer=tf.zeros_initializer())
    #
    # output_bias = tf.get_variable(
    #     "output_bias", [2], initializer=tf.zeros_initializer())
    # outputResult = tf.matmul(inputLayer, output_weights, transpose_b=True)
    # outputResult = tf.nn.bias_add(outputResult, output_bias)

    # W1 = tf.Variable(tf.random_normal([hidden_size, 2]), name="weight1")
    # b1 = tf.Variable(tf.zeros([1, 2]), name="bias1")
    # W2 = tf.Variable(tf.random_normal([50000, 2000]), name="weight2")
    # b2 = tf.Variable(tf.zeros([1, 2000]), name="bias2")
    # W3 = tf.Variable(tf.random_normal([2000, 2]), name="weight3")
    # b3 = tf.Variable(tf.zeros([2]), name="bias3"),
    # z1 = tf.matmul(output_layer, W1, transpose_b=False) + b1
    # z2 = tf.nn.relu(z1)
    # z3 = tf.matmul(z2, W2, transpose_b=False) + b2
    # z4 = tf.nn.relu(z3)
    # outputResult = tf.matmul(z4, W3, transpose_b=False) + b3
    #per_example_loss = tf.sqrt(tf.square(tf.subtract(outputResult, the_results)))
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, predi) = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, the_results,
        use_one_hot_embeddings)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      data_hook = tf.train.LoggingTensorHook({'input_id':input_ids, 'lonlat':the_results}, every_n_iter=1)
